...
var CollaborationContext = (0, import_react.createContext)({
  clientID: 0,
  color: randomEntry[1],
...
function useCollaborationContext(username, color) {
  const collabContext = (0, import_react.useContext)(CollaborationContext);
  if (username != null) {
...
function useYjsCollaboration(editor, id, provider, docMap, name, color, shouldBootstrap, binding, setDoc, cursorsContainerRef, initialEditorState, awarenessData) {
  const isReloadingDoc = (0, import_react2.useRef)(false);
  const connect = (0, import_react2.useCallback)(() => provider.connect(), [provider]);
  ...
  const cursorsContainer = (0, import_react2.useMemo)(() => {
    const ref = (element) => {
...
function useYjsHistory(editor, binding) {
  const undoManager = (0, import_react2.useMemo)(() => createUndoManager(binding, binding.root.getSharedType()), [binding]);
  (0, import_react2.useEffect)(() => {
    const undo = () => {
      undoManager.undo();
    };
  ...
  const clearHistory = (0, import_react2.useCallback)(() => {
    undoManager.clear();
  }, [undoManager]);
  React.useEffect(() => {
...
function CollaborationPlugin({
  ...
  const isBindingInitialized = (0, import_react2.useRef)(false);
  const isProviderInitialized = (0, import_react2.useRef)(false);
  ...
  const [doc, setDoc] = (0, import_react2.useState)(yjsDocMap.get(id));
  const [binding, setBinding] = (0, import_react2.useState)();
  (0, import_react2.useEffect)(() => {
    if (!provider) {
      return;
    }
